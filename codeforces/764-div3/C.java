         ...
         */
		 ...
		 while(t>0){
			...
			int arr[] = new int[n];
			 ...
			 printFoundPerm(arr,n);
			 t--;
		 }
        bw.flush();
        bw.close();
    }
    private static void printFoundPerm(int arr[], int n){
		/**
		  3 -> 3,1
		  6 -> 6,3,1
		  12-> 12,6,3,1
		  ...
		  choices
		   1. keep x
		   2. take any number from x/2
		  a. sort the arr
		  b. keep track of visited elem
		     - iterate from 0  to n for visited elems
			    - while elem > 1 and (elem > n or elem is seen) elem = elem /2
				- if already seen elem print "NO" nd return
				- mark elem seen
            - print the elem				
		...
		*/
        Arrays.sort(arr);
			 ...
			 for(int i=0;i<n;i++){
			     while(arr[i] > 1 && (arr[i] > n  || seen[arr[i]])){
			         arr[i] = arr[i]/2;
			     }
			     if(seen[arr[i]]){
